		...
		try{
			if (session.createCriteria(Zaposlenik.class).add(Restrictions.eq("id", ids)).setProjection(Projections.property("id")).uniqueResult() == null){
				if( session.createCriteria(Zaposlenik.class).add(Restrictions.like("username", username).ignoreCase()).list().size()!=0){
					JOptionPane.showMessageDialog(null, "Već postoji korisnik sa tim korisničkim imenom.");
					session.close();
			...
			Zaposlenik zap=(Zaposlenik) session.createCriteria(Zaposlenik.class).add(Restrictions.eq("id", ids)).uniqueResult();
			if(!username.equals(zap.getUsername()))
					if( session.createCriteria(Zaposlenik.class).add(Restrictions.like("username", username).ignoreCase()).list().size()!=0){
				JOptionPane.showMessageDialog(null, "Već postoji korisnik sa tim korisničkim imenom.");
				session.close();
